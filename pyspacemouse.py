...
if __name__ == "__main__":
    ...
    while True:
        ...
        if state and state != state_0:
            ...
            this_yaw = state.yaw
            if this_yaw > 0:
                this_w_up = int(round(this_yaw * max_wheel * sens))

                # adds to collected variable only when increasing
                if this_w_up >= last_w_up:
                    coll_w_up += this_w_up
            ...
            if this_yaw < 0:
                ...
                # adds to collected variable only when increasing
                if this_w_down >= last_w_down:
                    coll_w_down += this_w_down

                last_w_down = this_w_down
